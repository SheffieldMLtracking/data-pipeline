...
delete_script="/opt/data-pipeline/delete-old-files.sh"
...
do
  ...
  /usr/bin/rsync --archive --compress --update "$remote_host":"$remote_directory" "$local_directory" && \
  /usr/bin/ssh "$remote_host" "bash -s" < $delete_script
done
